...
from utils.tasks import get_tasks, get_dataset_list_glas
...
class GlasSegmentation(data.Dataset):
    cmap = ade_cmap()
    def __init__(self,
                 opts,
                 image_set='train',
        ...
        glas_root = f'./datasets/data/glas/{opts.task}_{opts.ckpt_postfix}'
        ...
        image_dir = os.path.join(self.root, 'glas')
        mask_dir = os.path.join(self.root, 'glas')
        
        assert os.path.exists(mask_dir), "annotations not found"
            
        self.target_cls = get_tasks('glas', self.task, cil_step)
        self.target_cls += [255] # including ignore index (255)
            
        if image_set=='test':
            file_names = get_dataset_list_glas('glas', self.task, cil_step, image_set, self.overlap)
            
        elif image_set == 'memory':
#             for s in range(cil_step):
#                 self.target_cls += get_tasks('glas', self.task, s)
            
            memory_json = os.path.join(glas_root, 'memory.json')
        ...
        self.masks = [os.path.join(mask_dir, x + "_anno.bmp") for x in file_names]
        self.file_names = file_names

        # class re-ordering
        all_steps = get_tasks('glas', self.task)
        all_classes = all_steps[0]
    ...
    def __getitem__(self, index):
        ...
        ######################
#         name1 = self.images[index].split("/")[-1].split(".")[0]
#         img.save(f"./check/{name1}.jpg")
        I = np.array(target)
        I[I > 0] = 1
        target = Image.fromarray(I)
#         name2 = self.masks[index].split("/")[-1].split(".")[0]
#         (Image.fromarray(I * 255)).save(f"./check/{name2}.jpg")
        ######################
