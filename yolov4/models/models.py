...
def create_modules(module_defs, img_size, cfg, opt):
    # Constructs module list of layer blocks from module configuration in module_defs

    img_size = [img_size] * 2 if isinstance(img_size, int) else img_size  # expand if necessary
    _ = module_defs.pop(0)  # cfg training hyperparams (unused)
    if opt.use_segmentation:
        output_filters = [4]  # input channels
    else:
...
class Darknet(nn.Module):
    ...
    def __init__(self, opt, img_size=(416, 416), verbose=False):
        super(Darknet, self).__init__()
        
        cfg = opt.cfg
        self.module_defs = parse_model_cfg(cfg)
        self.module_list, self.routs = create_modules(self.module_defs, img_size, cfg, opt)
